...
class TestCopyNetReader(AllenNlpTestCase):
    ...
    def test_source_to_target(self):
        ...
        tensor = source_to_target_field.as_tensor(
            source_to_target_field.get_padding_lengths()
        )
        check = np.array(
            [
        ...
        assert tensor[1].item() != self.vocab.get_token_index(
            DEFAULT_OOV_TOKEN, "target_tokens"
        )
